...
export default function DashboardPage() {
    ...
    const [processedNotifications, setProcessedNotifications] = useState<ProcessedNotification[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Fetch counts
            const { count: sCount } = await supabase.from('services').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
            const { count: rCount } = await supabase.from('requests').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
            const { count: asCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('seller_id', user.id).not('status', 'eq', 'completed');
            const { count: apCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('buyer_id', user.id).not('status', 'eq', 'completed');
            ...
            if (notifications) {
                for (const notification of notifications) {
                    if (notification.notification_type === 'new_message') {
                        if (!groupedNotifications.has(notification.order_id)) { groupedNotifications.set(notification.order_id, { ...notification, count: 0 }); }
                        groupedNotifications.get(notification.order_id).count++;
                    } else {
                        newProcessedNotifications.push({ order_id: notification.order_id, notification_type: notification.notification_type, content: `${notification.content} for "${notification.orders?.services?.title || 'a deleted service'}"`, created_at: notification.created_at });
                    }
                }
                groupedNotifications.forEach(group => {
                    newProcessedNotifications.push({ order_id: group.order_id, notification_type: 'new_message_group', content: `${group.count} new message${group.count > 1 ? 's' : ''} in your order for "${group.orders?.services?.title || 'a deleted service'}"`, created_at: group.created_at });
                });
                newProcessedNotifications.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    ...
    const handleNotificationClick = async (orderId: string, notificationType: string) => {
        if (!user) return;
        ...
        const { error } = await supabase
            ...
            .eq('recipient_id', user.id)
            .eq('order_id', orderId)
            .like('notification_type', notificationType === 'new_message_group' ? 'new_message' : notificationType);

        if (error) {
            console.error("Error marking notification as read:", error);
        }

        setProcessedNotifications(prev => prev.filter(n => n.order_id !== orderId || (notificationType !== 'new_message_group' && n.notification_type !== notificationType)));
                            ...
                            <div className="divide-y max-h-[420px] overflow-y-auto">
                                {processedNotifications.map((notif, index) => (
                                    <div 
                                        ...
                                        onClick={() => handleNotificationClick(notif.order_id, notif.notification_type)}
                                    >
